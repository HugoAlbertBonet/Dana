...
def extract_false_positives(image_path, target_mask_path, pred_mask_path, thr):
    ...
    # Extract FP pixel values
    fp_pixels = image[false_positive_mask == 255]

    # Plot histogram
    fig, ax = plt.subplots()
    colors = ('b', 'g', 'r')
    for i, color in enumerate(colors):
        ax.hist(fp_pixels[:, i], bins=256, range=(0, 256), alpha=0.5, label=f'{color.upper()} channel', color=color)
    ax.set_title("False Positives Pixel Histogram")
    ax.set_xlabel("Pixel Intensity")
    ax.set_ylabel("Frequency")
    ax.legend()
    hist_path = "fp_histogram.png"
    fig.savefig(hist_path)
    plt.close(fig)

    return final_path, hist_path
...
def extract_false_negatives(image_path, target_mask_path, pred_mask_path, thr):
    ...
    # Extract FN pixel values
    fn_pixels = image[false_negative_mask == 255]

    # Plot histogram
    fig, ax = plt.subplots()
    colors = ('b', 'g', 'r')
    for i, color in enumerate(colors):
        ax.hist(fn_pixels[:, i], bins=256, range=(0, 256), alpha=0.5, label=f'{color.upper()} channel', color=color)
    ax.set_title("False Negatives Pixel Histogram")
    ax.set_xlabel("Pixel Intensity")
    ax.set_ylabel("Frequency")
    ax.legend()
    hist_path = "fn_histogram.png"
    fig.savefig(hist_path)
    plt.close(fig)
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        # Fila 2.5: Histogramas
    with gr.Row():
        fp_hist = gr.Image(label="FP Histogram", type="filepath")
        fn_hist = gr.Image(label="FN Histogram", type="filepath")
    ...
    fp_button.click(extract_false_positives, inputs=[
                      img, target, pred, threshold], outputs=[fp, fp_hist])
    fn_button.click(extract_false_negatives, inputs=[
                      img, target, pred, threshold], outputs=[fn, fn_hist])
