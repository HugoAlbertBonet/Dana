...
from models import Generator, UNet, Colorizing, SwinIR, SegformerDANA
...
# Networks
model = SegformerDANA(token = secret["hf"], batch = opt.batchSize)
#detector = FaceExtractor()
#sam_segmentation_model = SAM2(device)
# "/home/salvem/jgalvan/ai-impainter/tools/trainer/yolov8/"
#yolo_model = YOLOV8(device=device)
#paths = glob.glob("/home/salvem/jgalvan/ai-impainter/tools/trainer/yolov8/runs/detect/*/weights/best.pt")
#yolo_path = paths[0]
...
criterion_sam = torch.nn.BCEWithLogitsLoss()
...
transforms_color = [transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1), transforms.RandomGrayscale(p=0.1)]
transforms_post = [transforms.ToTensor()]
dataloader = DataLoader(SegmentationDataset(opt.dataroot, transforms_=transforms_, transforms_color = transforms_color, transforms_post = transforms_post, unaligned=False, segformer = True, token = secret["hf"]),
                        batch_size=opt.batchSize, shuffle=True) #, num_workers=opt.n_cpu)
test_dataloader = DataLoader(SegmentationDataset(opt.dataroot, transforms_=[transforms.Resize((opt.size, opt.size))], transforms_post = transforms_post, unaligned=False, mode = "test", segformer = True, token = secret["hf"]),
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        image = batch["image"]
        mask = Variable(input_unet2.copy_(batch['mask'])).to(device)
        mask_people = Variable(input_unet2.copy_(batch['people'])).to(device)

        ###### main loop ######

        #loss
        pred = model(pixel_values = image.pixel_values.squeeze().to(device))
        loss = criterion(pred, mask)
        total_loss += loss.item()
        train_dice = dice(pred, mask)
        total_dice += train_dice
        
        loss_people = criterion(pred * (mask_people>0), mask * (mask_people>0))
        loss_false_negative = criterion(pred * (mask>0), mask)
        
          
        loss = loss + loss_people + loss_false_negative 
        loss.backward()
    ...
    with torch.no_grad():
        for j, val_batch in enumerate(test_dataloader):
            ...
            val_mask = Variable(input_unet2.copy_(val_batch['mask'])).to(device)
            val_mask_people = Variable(input_unet2.copy_(val_batch['people'])).to(device)

            val_pred = model(pixel_values = val_image.pixel_values.squeeze().to(device))
            ...
            val_loss_people = criterion(val_pred * (val_mask_people>0), val_mask * (val_mask_people>0))
            val_loss_false_negative = criterion(val_pred * (val_mask>0), val_mask)
          
            val_loss +=  val_loss_people + val_loss_false_negative
