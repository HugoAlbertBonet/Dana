...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        for im in image:
          ...
          mask_people = detector(im, prompt = "people", return_results = "mask", mask_multiplier = 255)
          mask_people = transforms.Compose([transforms.ToTensor()])(Image.fromarray(mask_people.astype(np.uint8))).to("cuda")
          mask_list.append(mask_people)
        mask_people = torch.stack(mask_list, dim = 0).to(device)
    ...
    with torch.no_grad():
        for j, val_batch in enumerate(test_dataloader):
            ...
            for im in val_image:
              ...
              mask_list.append(mask_people)
            mask_people = torch.stack(mask_list, dim = 0).to(device)
            val_loss_people = criterion(val_pred * (mask_people>0), val_mask * (mask_people>0))
